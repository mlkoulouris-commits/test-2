...
const getPaymentTitle = (paymentType: PaymentType): string => {
  switch (paymentType) {
    case "cash":
    ...
    case "no_payment":
      return "No Payment Transactions";
    default:
      return "Transactions";
  }
};

export const PaymentTransactionsModal = ({
  open,
  onOpenChange,
  date,
  paymentType,
  ...
  }, [open, date, paymentType, locationId, useFiscalDate, excludeVat, excludeNoPayment]);

  const loadTransactions = async () => {
    setLoading(true);
    const result = await getPaymentTransactions(date, paymentType, locationId, useFiscalDate, excludeVat, excludeNoPayment);
    if (result.data) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[90vw] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{getPaymentTitle(paymentType)}</DialogTitle>
          <DialogDescription>
            ...
            {totalDiscount > 0 && (
              <>
                ...
                <span className="text-orange-700 dark:text-orange-300">
                  -{formatAmount(totalDiscount)} BGN
                </span>
              </>
            )}
          </DialogDescription>
                    ...
                    >
                      ...
                      <TableCell className="text-right">
                        {transaction.totalDiscount > 0 ? (
                          <span className="text-orange-700 dark:text-orange-300">
                            -{formatAmount(transaction.totalDiscount)} BGN
                          </span>
                        ) : (
                      ...
                      <TableCell className="text-right">
                        {formatAmount(transaction.totalAmount)} BGN
                      </TableCell>
                    </TableRow>
                    {isExpanded && hasItems && (
                      <TableRow>
